...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        default="esp",
        choices=["none", "esp", "explicit", "attention", "semantic"],
        help=(
            "Spatial-prior module selector: Configurable ESP, "
            "esp/explicit=ExplicitSpatialPrior, attention=AttentionMap, "
            "semantic=ExplicitSemanticEnhancement, none=disable ESP."
        ),
    )
    parser.add_argument(
        "--coord_type",
        type=str,
        default="xy",
        choices=["none", "x", "y", "xy", "gauss"],
        help="Coordinate type for Explicit Spatial Prior. If none, then no coordinate is used.",
    )
    parser.add_argument(
        "--mask_mode",
        type=str,
        default="multiplication",
        choices=["addition", "multiplication", "concatenation"],
    )
    parser.add_argument(
        "--use_coordconv",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Whether to use a conv layer to project the concatenated features when using coordinate concatenation.",
    )
    parser.add_argument(
        "--gate_type",  
        type=str,
        default="learnable_temp_srcg",
        choices=["learnable_temp_srcg", "se", "cbam", "none"],
        help="Gate applied to NetVLAD descriptors. Use none to disable it.",
    )
    parser.add_argument(
        "--test_random_crop",
        type=float,
        default=0,
        help="Random resized crop for query images at test/inference time. 0 disables it; value x uses scale=(1-x, 1).",
    )
    parser.add_argument(
        "--test_random_rotation",
        type=float,
        default=0,
        help="Random rotation degrees for query images at test/inference time. 0 disables it.",
    )
    parser.add_argument(
        "--test_horizontal_flip",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Random horizontal flip for query images at test/inference time.",
    )
    parser.add_argument(
        "--use_extended_data",
        default=False,
    ...
    parser.add_argument(
        "--replace_padding_mode",
        type=str,
        choices=["zeros", "reflect", "replicate", "circular"],
        help="Replace ResNet zero padding with reflect padding.",
    )
    parser.add_argument(
        "--antiburst",
        action="store_true",
    ...
    parser.add_argument("--ab_fixed", action="store_true", help="ab params are init but arent nn.Parameter",)
    parser.add_argument("--num_pcs", type=int, default=2048, help="Use post pool PCA.")
    parser.add_argument("--wpca", action="store_true", default=False, help="Use post pool WPCA layer / cannot be used during training",)
    parser.add_argument("--use_vlad_local", action="store_true", default=False)
    ...
    parser.add_argument(
        ...
        type=int,
        default=32,
        help="Input feature map height for MixVPR.",
    )
    parser.add_argument(
        "--mixvpr_in_w",
        type=int,
        default=32,
        help="Input feature map width for MixVPR.",
    )
    parser.add_argument(
        "--mixvpr_out_channels",
        type=int,
        default=1024,
        help="MixVPR channel-wise projection dimension.",
    )
    parser.add_argument(
        "--mixvpr_mix_depth",
        type=int,
        default=4,
        help="Number of stacked FeatureMixer layers in MixVPR.",
    )
    parser.add_argument(
        "--mixvpr_mlp_ratio",
        type=float,
        default=1,
        help="MLP hidden width ratio in MixVPR FeatureMixer.",
    )
    parser.add_argument(
        "--mixvpr_out_rows",
        type=int,
        default=4,
        help="Number of rows for MixVPR output aggregation.",
    )
    ...
    parser.add_argument("--seed", type=int, default=0) # 0,1,42
    parser.add_argument(
        "--resume",
        type=str,
        default=None,
    ...
    parser.add_argument(
        "--dataset_name",
        type=str,
        default="satellite_0_thermalmapping_135_100",
        choices=["satellite_0_thermalmapping_135_100", "DSM", "tirmss", "VEDAI", "NEU"],
    ...
    parser.add_argument(
        ...
        type=str,
        default="auto",
        choices=["auto", "distance", "exact"],
        help="How positives are selected from UTM labels: distance threshold, exact label match, or automatic.",
    )
    parser.add_argument(
        "--pca_dataset_folder",
        type=str,
    ...
    args.use_esp = args.use_esp and args.esp_type != "none"
