...
from model.semantic_recalibration import build_vlad_gate
from model.vlad_buff import getWeights
...
class FeatureMixerLayer(nn.Module):
    def __init__(self, in_dim, mlp_ratio=1):
        super().__init__()
        hidden_dim = max(int(in_dim * mlp_ratio), 1)
...
class MixVPR(nn.Module):
    """All-MLP feature aggregation for visual place recognition."""
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        if C != self.in_channels or H != self.in_h or W != self.in_w:
            raise ValueError(
                "MixVPR input shape mismatch: "
                f"expected (B, {self.in_channels}, {self.in_h}, {self.in_w}), "
                f"got {tuple(x.shape)}"
            )
        x = x.flatten(2)
        x = self.mix(x)
...
class NetVLAD(nn.Module):
    ...
    def __init__(
        ...
        self.gate_type = "learnable_temp_srcg" if args is None else getattr(args, "gate_type", "learnable_temp_srcg")
        self.antiburst = False if args is None else getattr(args, "antiburst", False)
        self.gate = build_vlad_gate(self.gate_type, C=dim, drop_path=0.2)
        if self.antiburst:
            ab_w = getattr(args, "ab_w", 8.0)
            ab_b = getattr(args, "ab_b", 7.0)
            ab_p = getattr(args, "ab_p", 1.0)
            ab_fixed = getattr(args, "ab_fixed", False)

            if ab_fixed:
                self.register_buffer("ab_params", torch.tensor([ab_w, ab_b, ab_p], dtype=torch.float32))
            else:
                self.ab_params = nn.Parameter(torch.tensor([ab_w, ab_b, ab_p], dtype=torch.float32))
    ...
    def forward(self, x, epoch=None):
        ...
        soft_assign = self.conv(x).view(B, K, -1)
        ...
        if self.antiburst:
            ...
            w_burst = getWeights(selfDis, ab_params)
            soft_assign = soft_assign / w_burst[:, None, :]

        vlad = torch.zeros(
            [B, self.clusters_num, C], dtype=x_flatten.dtype, device=x_flatten.device
        )
        for k in range(self.clusters_num):
            residual_dc = x_flatten.unsqueeze(0).permute(1, 0, 2, 3) - self.centroids[k : k + 1, :
            ].expand(x_flatten.size(-1), -1, -1).permute(1, 2, 0).unsqueeze(0)
            residual_dc = residual_dc * soft_assign[:, k : k + 1, :].unsqueeze(2)

            vlad[:, k : k + 1, :] = residual_dc.sum(-1)
    ...
    def initialize_netvlad_layer(self, args, cluster_ds, backbone):
        ...
        try:
            kmeans = faiss.Kmeans(self.dim, self.clusters_num, niter=100, verbose=True)
...
class CRNModule(nn.Module):
    ...
    def forward(self, x):
        ...
        mask = self.upsample(w, scale_factor=2, mode='bilinear')
...
class CRN(NetVLAD):
    ...
    def forward(self, x):
        ...
        for D in range(self.clusters_num):
