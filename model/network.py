...
from model.spatial_prior import (
    AttentionMap,
    ExplicitSemanticEnhancement,
    ExplicitSpatialPrior,
)
from model.pix2pix_networks.networks import UnetGenerator, GANLoss, NLayerDiscriminator, get_scheduler
from model.sync_batchnorm import convert_model

from model.padding import replace_padding_mode
...
def get_spatial_prior(args):
    """Build the selected spatial-prior module.

    The project environment uses Python 3.9, so this uses a registry-style
    dispatch instead of Python 3.10 ``match/case`` syntax.
    """
    if not args.use_esp:
        return None

    esp_type = args.esp_type.lower()
    input_features_dim = args.features_dim

    def build_ablation():
        return ExplicitSpatialPrior(
            input_features_dim,
    ...
    builders = {
        "esp": lambda: ExplicitSpatialPrior(input_features_dim),
        "explicit": lambda: ExplicitSpatialPrior(input_features_dim),
        "attention": lambda: AttentionMap(input_features_dim),
        "semantic": lambda: ExplicitSemanticEnhancement(input_features_dim),
    }

    if esp_type not in builders:
        raise ValueError(
            f"Unsupported esp_type: {args.esp_type}. "
            f"Choose from {sorted(builders)} or disable with esp_type='none'."
        )

    spatial_prior = builders[esp_type]()

    if (
        esp_type == "esp"
        and not args.use_coordconv
        and args.mask_mode == "concatenation"
    ):
        args.features_dim = input_features_dim + 1
...
class GeoLocalizationNet(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.self_att = False
        self.DA = args.DA
        self.ESP = get_spatial_prior(args)
        if args.aggregation == "mixvpr":
            args.mixvpr_in_channels = args.features_dim
            args.features_dim = args.mixvpr_out_channels * args.mixvpr_out_rows
        ...
        if args.fc_output_dim != None:
            ...
            logging.debug(f"fc_output_dim: {args.fc_output_dim}")
        if args.aggregation in ["netvlad", "crn"] and args.conv_output_dim != None:
            ...
            logging.debug(f"Input feature channels: {args.features_dim}")
            if args.add_bn:
                    self.conv_layer = nn.Sequential(
                        nn.Conv2d(args.features_dim, actual_conv_output_dim, 1, bias=False),
        ...
        if args.wpca:
            if args.nv_pca is not None:
                ...
                netvlad_output_dim = args.dim
            netvlad_output_dim = args.clusters_num * netvlad_output_dim
            pca_conv = nn.Conv2d(
                netvlad_output_dim,
                args.num_pcs,
                kernel_size=(1, 1),
    ...
    def create_domain_classifier(self, args):
        ...
        if self.DA.startswith('DANN_before'):
            ...
            if args.aggregation == "mixvpr":
                dann_input_dim = args.features_dim
            else:
                dann_input_dim = args.conv_output_dim
            domain_classifier = nn.Sequential(nn.Linear(dann_input_dim, 100, bias=False),
                                                   nn.BatchNorm1d(100),
                                                   nn.ReLU(True),
                                                   nn.Linear(100, 2),
                                                   nn.LogSoftmax(dim=1))
    ...
    def forward(self, x, is_train=False, alpha=1.0, epoch=None):
        x = self.backbone(x)
    ...
    if args.aggregation == "gem":
        ...
        if hasattr(args, "conv_output_dim") and args.conv_output_dim is not None:
            ...
            return aggregation.NetVLAD(clusters_num=args.netvlad_clusters, dim=args.features_dim,
                                            work_with_tokens=args.work_with_tokens, args=args)
        ...
        return aggregation.MixVPR(
            in_channels=args.mixvpr_in_channels,
            in_h=args.mixvpr_in_h,
            in_w=args.mixvpr_in_w,
            out_channels=args.mixvpr_out_channels,
            mix_depth=args.mixvpr_mix_depth,
            mlp_ratio=args.mixvpr_mlp_ratio,
            out_rows=args.mixvpr_out_rows,
        )
...
def get_backbone(args):
    ...
    if args.backbone.startswith("resnet"):
        ...
        if hasattr(args, 'replace_padding_mode') and args.replace_padding_mode is not None:
            backbone = replace_padding_mode(backbone, mode=args.replace_padding_mode)
