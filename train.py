...
train_ds = None
train_ds = datasets_ws.TripletsDataset(args, args.datasets_folder, args.dataset_name, "train", args.negs_num_per_query)
...
if args.aggregation == "crn":
    ...
    if args.optim == "adam":
        if args.separate_branch:
            if domain_classifier is not None:
                optimizer = torch.optim.Adam(
                    list(model.parameters()) + list(model_db.parameters()) + list(domain_classifier.parameters()),
            ...
            if domain_classifier is not None:
                optimizer = torch.optim.Adam(list(model.parameters()) + list(domain_classifier.parameters()),
                                             lr=args.lr, weight_decay=args.weight_decay)
            else:
        ...
        if args.separate_branch:
            if domain_classifier is not None:
                optimizer = torch.optim.SGD(
                    list(model.parameters()) + list(model_db.parameters()) + list(domain_classifier.parameters()),
                    lr=args.lr, momentum=0.9, weight_decay=0.001)
            else:
                optimizer = torch.optim.SGD(list(model.parameters()) + list(model_db.parameters()), lr=args.lr,
                                            momentum=0.9, weight_decay=0.001)
        else:
            if domain_classifier is not None:
                optimizer = torch.optim.SGD(list(model.parameters()) + list(domain_classifier.parameters()), lr=args.lr,
                                            momentum=0.9, weight_decay=0.001)
            else:
    ...
    for loop_num in range(loops_num):
        ...
        for images, triplets_local_indexes, _ in tqdm(triplets_dl, ncols=100):
            ...
            if args.separate_branch:
                ...
                if args.DA.startswith('DANN'):
                    database_feature, database_reverse_x = model_db(database_images.to(args.device), is_train=True,
                                                                    alpha=alpha)
...
if args.separate_branch:
    recalls, recalls_str = test.test(
        args, test_ds, model, model_db=model_db, test_method=args.test_method, visualize=True)
else:
    recalls, recalls_str = test.test(
        args, test_ds, model, model_db=model, test_method=args.test_method, visualize=True)
